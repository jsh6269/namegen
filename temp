...
__global__ void gpu_mat_temp(float *gpu_input1, float *gpu_input2, float *temp, size_t N_, size_t K_) {
  int tidx = blockIdx.x * blockDim.x + threadIdx.x;
  int tidy = blockIdx.y * blockDim.y + threadIdx.y;
  if(tidx >= N_ || tidy >= K_){
    return;
  }
  temp[N_ * tidy + tidx] = gpu_input1[tidx * K_ + tidy] * gpu_input2[tidy];
}



__global__ void gpu_matvec(float *gpu_input1, float *gpu_output, size_t N_, size_t K_) {
  int tidx = blockIdx.x * blockDim.x + threadIdx.x;
  if(tidx >= N_){
    return;
  }
...
void matvec(Tensor *input1, Tensor *input2, Tensor *output) {
  ...
  dim3 gridDim((N_+32-1)/32, (K_+32-1)/32);
  dim3 blockDim(32, 32);
  gpu_mat_temp<<<gridDim, blockDim>>>(input1->buf, input2->buf, f1048576, N_, K_);

  dim3 gridDim2((N_+512-1)/512);
  dim3 blockDim2(512);
  gpu_matvec<<<gridDim2, blockDim2>>>(f1048576, output->buf, N_, K_);
}
...
//  rfloats = new Tensor({N * MAX_LEN});
  ...
  CHECK_CUDA(cudaMalloc(&f1048576, sizeof(float) * 1048576));
